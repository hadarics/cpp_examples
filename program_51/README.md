# TCP/IP processz alapú szerver mintapélda
* [Source](#source)
    ...
    while (1) {
        ...
        if (clientInfo.status==TCP_CONNECTION_OK) {
            ...
            if (client_pid==0) {
                ...
                while((bytes=recv(clientInfo.fd, in_message, max_buffer_size,0))) {
                    ...
                    map <string, unsigned>::iterator it = population.find(in_msg);
                    ...
                    send(clientInfo.fd, out_message, strlen (out_message), 0);
                }
                signal(SIGCHLD, signal_handler);
                close(clientInfo.fd);
...
## Description
> A program megnyitja az 5678/tcp portot, és figyeli a bejövő kapcsolatokat.  

> A program egyes kliens számára új processzt indít.  

> Nyitott portok listázása:  
<code>ss -ltpn</code>

> A kapcsolódás tesztelhető nc -vel, pl.:  
<code>nc 192.168.1.100 5678 </code>
